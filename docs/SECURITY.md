...
3. **Budget is enforced by bridge, not by honor system.** Container can't spoof spend.
4. **Kill-switch lives on host filesystem.** A compromised container cannot delete it.
...
7. **Audit log is append-only and mirrored off-box weekly.**
8. **Every diff touching `safety/**`, `claude-bridge/src/bridge/{budget,policy,audit,approval}.py`, or `CLAUDE.md` requires human approval via Telegram — no auto-merge.**
